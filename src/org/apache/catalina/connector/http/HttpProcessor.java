...
/***
 * 1、将HttpConnector接受到的socket转化为本地的socket（assign）,同时唤醒线程（available）.(改assign方法在HttpConnector线程中调用)
 * 2、新建HttpProcessor对象的时候就开启了线程run，解析socket请求，request/response(process)，
 * 只有接受到socket后才能唤醒run。
 * 解析完成后回收该HttpProcessor对象，此时线程被阻塞，等待新的http请求。（即过程1）
 *
 * 生产者消费者的模型，相当于生产者与消费者个数相当。此处assign相当于生产者（生产一个socket）
 * 线程run为消费者（消费一个socket）
 *
 */
final class HttpProcessor
     ...
     * 返回局部变量socket而不是成员变量，是因为使用局部变量可以在当前Socket对象处理完之前，继续接受下一个Socket对象
     */
